...
public class BattleSceneStarter : MonoBehaviour // 전투 씬 시작 관리 클래스
{
    ...
    private void FindReferencesIfNeeded() // 주요 매니저와 플레이어 관련 참조 자동 연결
    {
        if (battleManager == null) battleManager = FindFirstObjectByType<BattleManager>(); // 전투 관리 참조 확인 및 연결
        if (enemySpawner  == null)  enemySpawner = FindFirstObjectByType<EnemySpawner>(); // 적 생성 관리 참조 확인 및 연결
        if (relicManager  == null)  relicManager = FindFirstObjectByType<RelicManager>(); // 유물 관리 참조 확인 및 연결
        if (playerStats   == null)   playerStats = FindFirstObjectByType<PlayerStats>(); // 플레이어 능력치 참조 확인 및 연결
        if (cardManager   == null)   cardManager = FindFirstObjectByType<CardManager>(); // 카드 관리 참조 확인 및 연결
    }
    ...
    private void StartSelectedBattle() // 선택 전투 시작 함수
    {
        ...
        Debug.Log("Battle Scene Start : " + GameFlowManager.Instance.selectedNodeType + " / " + GameFlowManager.Instance.selectedBattleType); 
        // 전투 시작 로그
