...
public class RoomSceneController : MonoBehaviour // 실제 방 프리팹 생성과 이동을 관리하는 클래스
{
    ...
    public GameObject defaultRoomPrefab; // 모든 타입에 사용할 수 있는 기본 방 프리팹
    ...
    {
        GameObject selectedRoomPrefab = SelectRoomPrefabForCurrentRoom(); // 현재 방 타입에 맞는 프리팹 선택
    ...
    private GameObject SelectRoomPrefabForCurrentRoom() // 현재 방 타입에 맞는 프리팹 선택 함수
    {
        RoomData currentRoomData = GetCurrentRoomData(); // 현재 방 데이터 가져오기

        GameObject[] candidatePrefabs = GetCandidatePrefabs(currentRoomData); // 후보 프리팹 목록 가져오기

        if (candidatePrefabs == null || candidatePrefabs.Length == 0) // 후보 프리팹 확인
        {
            return defaultRoomPrefab; // 후보가 없으면 기본 방 반환
        }
        ...
        {
            return defaultRoomPrefab; // 비어 있으면 기본 방 반환
        }
    ...
    private GameObject[] GetCandidatePrefabs(RoomData roomData) // 방 타입에 따른 후보 프리팹 반환 함수
    {
        ...
        if (roomData.roomType == RoomType.NormalBattle) // 일반 전투방 확인
        {
            return normalBattleRoomPrefabs; // 일반 전투방 프리팹 반환
        }

        if (roomData.roomType == RoomType.EliteBattle) // 정예 전투방 확인
        {
        ...
        if (roomData.roomType == RoomType.MidBoss) // 중간 보스방 확인
        {
            return midBossRoomPrefabs; // 중간 보스방 프리팹 반환
        }

        if (roomData.roomType == RoomType.FinalBoss) // 최종 보스방 확인
        {
