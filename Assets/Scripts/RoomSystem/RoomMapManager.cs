...
public class RoomMapManager : MonoBehaviour // 랜덤 방 맵 생성과 현재 방 이동을 관리하는 클래스
{
    ...
    public int maxFloorNumber = 15; // 최대 층 번호
    ...
    private void AssignSpecialEndRoom() // 끝방 지정 함수
    {
        ...
        farthestRoom.isEndRoom = true; // 가장 먼 방을 현재 층의 끝방으로 표시
    ...
    public bool CanGoNextFloor() // 다음층으로 이동 가능한지 확인하는 함수
    {
        return currentFloorNumber < maxFloorNumber; // 현재 층이 최대 층보다 낮은지 반환
    }

    public void GoToNextFloor() // 다음층으로 이동하는 함수
    {
        if (!CanGoNextFloor()) // 다음층 이동 가능 여부 확인
        {
            Debug.Log("Final floor cleared. Game clear flow will be added later."); // 최종층 클리어 로그
            return; // 다음층 이동 중단
        }

        int nextFloorNumber = currentFloorNumber + 1; // 다음층 번호 계산

        GenerateFloor(nextFloorNumber); // 다음층 맵 생성
    }
}