...
public class EnemySpawner : MonoBehaviour
{
    ...
    public void SpawnEnemies(BattleManager battleManager, int battleNumber) // 전투 번호에 맞춰 적을 생성한다.
    {
        ...
        int enemyCount = GetEnemyCount(battleNumber); // 이번 전투의 적 수를 계산한다.
        int enemyHealth = GetEnemyHealth(battleNumber); // 이번 전투의 적 체력을 계산한다.
        ...
        for (int i = 0; i < enemyCount; i++) // 생성할 적 수만큼 반복한다.
        {
            ...
            enemyHealthComponent.SetMaxHealth(enemyHealth); // 전투 번호에 맞는 체력을 설정한다.
    ...
    {
        int count = baseEnemyCount + ((battleNumber - 1) * enemyCountIncreasePerBattle); // 적 수를 계산한다.
        return Mathf.Max(count, 1); // 최소 1마리는 나오도록 반환한다.
    }

    private int GetEnemyHealth(int battleNumber) // 전투 번호에 따른 적 체력을 계산한다.
    {
        int health = baseEnemyHealth + ((battleNumber - 1) * enemyHealthIncreasePerBattle); // 적 체력을 계산한다.
